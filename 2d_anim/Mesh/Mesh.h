...
public:
    ...
    void drawCurve(OpenGP::Mat4x4 Model, OpenGP::Mat4x4 View, OpenGP::Mat4x4 Projection){
        ...
        check_error_gl();
        ///--- Draw
        glDrawElements(GL_TRIANGLE_FAN, /*#vertices*/ numVertices,
                    GL_UNSIGNED_INT,
                    0 /*ZERO_BUFFER_OFFSET*/);
        check_error_gl();
    ...
    void draw(OpenGP::Mat4x4 Model, int c) {
        OpenGP::Mat4x4 id = OpenGP::Mat4x4::Identity();
        if (c){
           drawCurve(Model, id, id); //draw with triangle fan
        }else{
            draw(Model, id, id); //draw with triangle
        }
